...
MODEL_PATH = os.path.join(
    SCRIPT_DIR, "model", "best_deeplabv3plus_mobilenet_cityscapes_os16.pth")
IMAGE_PATH = os.path.join(SCRIPT_DIR, "client_images", "temp.jpg")
NUM_CLASSES = 19
OUTPUT_STRIDE = 16

def replace_with_empty_file(file_path):
    ...
    if os.path.exists(file_path):
        ...
        return  # Stop the function if there's no file to delete

    # Create an empty file at the same location
...
# replace_with_empty_file(file_path)

def segment_and_overlay(image_path):
    # Load the model
    model = modeling.__dict__["deeplabv3plus_mobilenet"](
        num_classes=NUM_CLASSES, output_stride=OUTPUT_STRIDE)
    model.load_state_dict(torch.load(
        MODEL_PATH, map_location=torch.device('cpu'))["model_state"])
    model.eval()
    ...
    colorized_preds = colorized_preds.resize(
    ...
    limited_colors = [[0, 0, 142], [244, 35, 232], [
        128, 64, 128], [250, 170, 30], [220, 220, 0]]
...
output_path_blurred = os.path.join(
    SCRIPT_DIR, "test_result", f"output_{formatted_date_time}.png")
